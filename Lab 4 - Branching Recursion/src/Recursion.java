...
public class Recursion {
    ...
    public static int mult(int j, int k){
         ...
         //return ((k > 0) ? j + mult(j, k - 1) : k + mult(k ,j - 1));
    }
    ...
    public static boolean isPalindrome(String word){
         if (word.length() < 2)
        	 return true;
         else if (word.toLowerCase().charAt(0) != word.toLowerCase().charAt(word.length() - 1))
        	 return false;
         else 
    ...
    public static int longestWordLength(String words){
    	...
    	if (wordArr.length < 2)
    		...
    		if (wordArr[0].length() < wordArr[1].length())
    			return longestWordLength(words.substring(words.indexOf(wordArr[1])));
    		else
    			// words.indexOf(wordArr[1]) - Does not work for some reason.
    			return longestWordLength(words.substring(0, wordArr[0].length()) + words.substring(wordArr[0].length() + wordArr[1].length() + 1));
    	}
    }
    ...
    public static String dedupeChars(String word){
        if (word.length() < 2)
        	...
        	char c1 = word.toLowerCase().charAt(0);
        	char c2 = word.toLowerCase().charAt(1);
        	
        	if (c1 == c2) {
        		return dedupeChars(word.charAt(0) + word.substring(2));
        	}
        	else 
    ...
    public static void main(String [] args){
        ...
        System.out.printf("mult(-2, -6) should be 12 -> %d\n", mult(-2, -6));
        System.out.println();
